...
for (i in 1:25) {
  merval[[paste0("lag_", i)]] <- lag(merval$log_return, i)
}
...
x <- as.matrix(merval[, paste0("lag_", 1:25)])
y <- merval$lead_return
...
p1 <- ggplot(bootstrap_df, aes(x = RMSE)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "darkblue", color = "white", size = 0.1) +
...
ggsave("merval rr.pdf", plot = plot, device = "pdf", dpi = 320)
...
strategy_returns <- merval$return[test_indices] * lag(signal, default = 0)
...
market_cumulative_returns <- cumprod(1 + merval$return[test_indices]) - 1

# Create a ggplot of cumulative returns
ggplot() +
  geom_line(aes(x = merval$date[test_indices], y = market_cumulative_returns, color = "Buy-and-Hold"), linewidth = 0.4) +
  geom_line(aes(x = merval$date[test_indices], y = strategy_cumulative_returns, color = "RR"), linewidth = 0.4) +
  scale_color_manual(values = c("Buy-and-Hold" = "darkblue", "RR" = "forestgreen")) +
  ggthemes::theme_base() +
  labs(x = "", y = "") +
...
ggsave("merval rr cum.pdf", dpi = 320, width = 4, height = 3)
dev.off()

# Compare cumulative returns
plot(merval$date[test_indices], market_cumulative_returns, type = "l", col = "steelblue3", lwd = 1.5, 
     main = "Cumulative Returns: Strategy vs Market", ylab = "Cumulative Return", xlab = "Time")
lines(merval$date[test_indices], strategy_cumulative_returns, col = "darkorange", lwd = 1.5)
legend("topleft", legend = c("Market", "Strategy"), col = c("steelblue3", "darkorange"), lty = 1, lwd = 1.5)
