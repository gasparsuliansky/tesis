...
acf_values <- acf(sp$log_return, lag.max = 10, plot = FALSE)  # Ensure plot = FALSE to get values without plotting

# Convert to a data frame for ggplot
acf_df <- data.frame(lag = acf_values$lag, acf = acf_values$acf)

# Calculate the confidence interval for white noise
conf_level <- 1.96 / sqrt(length(sp$log_return))

# Create ACF plot using ggplot2
ggplot(acf_df, aes(x = lag, y = acf)) +
  geom_line(linetype = "dashed", linewidth = 0.3, col = "darkred") +
  geom_bar(stat = "identity", fill = "darkred", width = 0.025) +
...
ggsave("s&p acf plot.pdf", dpi = 320, width = 4, height = 3)

# Compute significance
acf_values <- acf(sp$log_return, lag.max = 10, plot = FALSE)$acf[-1]

# Determine the sample size
N <- length(sp$log_return)
...
box_pierce <- Box.test(sp$log_return, lag = 10, type = "Box-Pierce")
box_pierce
ifelse(box_pierce$p.value < 0.05, paste("H1"), paste("H0"))

ljung_box <- Box.test(sp$log_return, lag = 10, type = "Ljung-Box")
ljung_box
...
for (i in 1:length(sp_list)) {
  sp_period <- sp_list[[i]]  # Get the current dataset for the period
  period <- periods[i]  # Get the current period
  
  # Calculate the autocorrelation function up to lag 10
  acf_values <- acf(sp_period$log_return, plot = FALSE, lag.max = 10)$acf[-1]  # Exclude lag 0
  
  # Determine the sample size
  N <- length(sp_period$log_return)
  ...
  # Box-Pierce Test
  box_pierce <- Box.test(sp_period$log_return, lag = 10, type = "Box-Pierce")
  ...
  # Ljung-Box Test
  ljung_box <- Box.test(sp_period$log_return, lag = 10, type = "Ljung-Box")
...
qqnorm(sp$log_return, main = "QQ Plot of S&P 500 Returns", pch = 19, col = alpha("darkred", 0.25))
qqline(sp$log_return, lty = 2)

# Create the plot
ggplot(sp, aes(sample = log_return)) +
  stat_qq(color = "darkred", alpha = 0.25, shape = 19) +
...
anderson_darling <- ad.test(sp$log_return)
anderson_darling
ifelse(anderson_darling$p.value < 0.05, paste("H1"), paste("H0"))

jarque_bera <- jarque.bera.test(sp$log_return)
...
for (i in 1:length(sp_list)) {
  sp_period <- sp_list[[i]]  # Get the dataset for the current period
  period <- periods[i]       # Get the label for the current period
  
  # QQ Plot
  qqnorm(sp_period$log_return, main = paste("QQ Plot of S&P 500 Returns -", period), pch = 19, col = alpha("darkred", 0.25))
  qqline(sp_period$log_return, lty = 2)
  
  # Anderson-Darling Test
  anderson_darling <- ad.test(sp_period$log_return)
  print(paste("Period:", period, 
  ...
  # Jarque-Bera Test
  jarque_bera <- jarque.bera.test(sp_period$log_return)
  print(paste("Period:", period, 
              "Jarque-Bera statistic:", round(jarque_bera$statistic, 4), 
              "p-value:", format(jarque_bera$p.value, scientific = TRUE, digits = 3)))
  
  # Shapiro-Wilk Test
  shapiro_wilk <- shapiro.test(sp_period$log_return)
  print(paste("Period:", period, 
...
augmented_dickey_fuller <- adf.test(sp$log_return)
...
for (i in 1:length(sp_list)) {
  ...
  augmented_dickey_fuller <- adf.test(sp_period$log_return)
...
for (i in 1:length(sp_list)) {
  ...
  pos_count <- sum(sp_period$sign_return == 1)
  zero_count <- sum(sp_period$sign_return == 0)
  neg_count <- sum(sp_period$sign_return == -1)

  # Calculate the number of runs
  runs <- sum(diff(sp_period$sign_return) != 0) + 1
  n <- length(sp_period$sign_return)
...
lo_mac <- Lo.Mac(sp$log_return, kvec = lag_periods)
...
for (i in 1:length(sp_list)) {
  ...
  # Lo-MacKinlay Test
  lo_mac <- Lo.Mac(sp_period$log_return, kvec = c(2, 5, 10))
  z_stat_m1 <- lo_mac$Stats[, "M1"]
  z_stat_m2 <- lo_mac$Stats[, "M2"]
...
hurst_exp <- hurstexp(sp$log_return, display = F)
...
for (i in 1:n_sim) {
  simulated_data <- fracdiff.sim(n = length(sp$log_return), d = 0)$series
...
for (i in 1:length(sp_list)) {
  ...
  hurst_exp <- hurstexp(sp_period$log_return, display = F)
  ...
  for (j in 1:n_sim) {
    simulated_data <- fracdiff.sim(n = length(sp_period$log_return), d = 0)$series
