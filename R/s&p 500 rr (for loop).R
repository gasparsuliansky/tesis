...
for (j in 1:length(sp_list)) {
  ...
  for (i in 1:25) {
    sp_period[[paste0("lag_", i)]] <- lag(sp_period$log_return, i)
  }
  ...
  x <- as.matrix(sp_period[, paste0("lag_", 1:25)])
  y <- sp_period$lead_return
  ...
  if (sd(as.numeric(ridge_predictions)) < 1e-10) {
    ...
    sp_period$MA5 <- rollmean(sp_period$close, k = 5, fill = NA, align = "right")
    sp_period$MA20 <- rollmean(sp_period$close, k = 20, fill = NA, align = "right")
    
    # Generate signals based on moving average crossover
    sp_period$signal <- ifelse(sp_period$MA5 > sp_period$MA20, 1, 
                               ifelse(sp_period$MA5 < sp_period$MA20, -1, 0))
    
    # Use the signals for the test period
    signal <- sp_period$signal[test_indices]
  } else {
  ...
  strategy_returns <- sp_period$return[test_indices] * lag(signal, default = 0)
  ...
  market_cumulative_returns <- cumprod(1 + sp_period$return[test_indices]) - 1
  
  # Compare cumulative returns
  plot(sp_period$date[test_indices], market_cumulative_returns, type = "l", col = "steelblue3", lwd = 2, 
       main = paste("Cumulative Returns: Strategy vs Market (", periods[j], ")", sep = ""), 
       ylab = "Cumulative Return", xlab = "Time")
  lines(sp_period$date[test_indices], strategy_cumulative_returns, col = "darkorange", lwd = 2)
